...
namespace TrinityCore.GameClient.Net.Lib.Clients
{
    public class ExtendableClient : BaseClient
    {
        ...
        internal ExtendableClient(string host, int port, string login, string password) : base(host, port, login, password)
        {
            Components = new List<GameComponent>();
        }

        internal T AddComponent<T>(T component) where T : GameComponent, new()
        {
            if (WorldClient != null)
            {
