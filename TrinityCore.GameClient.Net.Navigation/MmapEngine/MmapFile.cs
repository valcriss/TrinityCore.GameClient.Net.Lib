...
namespace TrinityCore.GameClient.Net.Navigation.MmapEngine
{
    public class MmapFile
    {
        ...
        public string GetMmapTileKeyFromVector3(Vector3 position)
        {
            ...
            var key = MapId.ToString("000") + tileX.ToString("00") + tileY.ToString("00");
            NavTrace.WriteLine(
                $"NAV MMAP TILEKEY map={MapId} world=({position.X:F3},{position.Y:F3},{position.Z:F3}) " +
                $"tileWidth={TileWidth:F3} tileHeight={TileHeight:F3} tileX={tileX} tileY={tileY} key={key}");
            return key;
        }
